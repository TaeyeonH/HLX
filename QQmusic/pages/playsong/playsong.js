...
Page({
  ...
  changeOption:function(index,id,mid,albummid,songlists){
    //  that.getLyric(id);
     util.getSongInfo(id,mid,function(data){
       that.setData({title:data.title,singer:data.singer});
     });
     that.setData({
       selectedIndex:index,
       songlists:songlists,
  ...
  changeSong: function (ev) {
    ...
    // 修改配置
    var id = currentData.data.songid;
    var mid = currentData.data.songmid;
    
    if (that.data.songFrom === 'toplist') {
      var albummid = currentData.data.albummid;
      id = currentData.data.songid;
    } else {
      var albummid = currentData.data.albummid;
    }
